...
if __name__ == "__main__":
    # main()
    data_recoder = np.load("data_recoder.npy")
    time_recoder = np.load("time_recoder.npy")
    ...
    zero_hour = datetime(2019,7,15)
    real_datetime = zero_hour

    location_log = []

    prev_index = -1
    for day in range(99, -1, -1):
        for time in range(144):
            real_datetime = real_datetime - timedelta(minutes=10)
            index = int(data_recoder[day,time,0])
            # x, y = zahyou_index_ls[index]
            if index != prev_index:
                x, y = zahyou_index_ls[index]
                location_log.append({"date": real_datetime.timestamp(), "lat": x, "lng": y,})
                prev_index = index
    print( len(location_log) )
    location_log.reverse()
    import json
    json_str = json.dumps(location_log)
    ...
    # place = rs.suggest_place #.recommend()
